...
function HRManagementTimekeeping() {
    ...
    const { data: getAllDeptInOrgUnits = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await orgUnitApi.getAllDeptInOrgUnit()
            const mappedData = res?.data?.data?.map((item: { Id: { toString: () => never; }; Name: never; }) => ({
                id: item.Id.toString(),
                label: item.Name,
                type: "department"
            }));
    ...
    const handleCheckedChange = useCallback((nodes: TreeNode[]) => {
        ...
        const jobTitleNodes = nodes.filter((node) => node.type === "jobtitle");
        setCheckedIds(jobTitleNodes.map((n) => n.id));
    }, []);
    ...
    const handleSaveUserHavePermissionMngTimeKeeping = async () => {
        ...
        try {
            await userApi.UpdateUserHavePermissionMngTimeKeeping(valueArray);
            ShowToast("Sucess")
        }
        catch (err) {
    ...
    const handleSaveUserMngTimeKeeping = () => {
        console.log(currentSelectedUser, checkedIds);
    }
        ...
        <div className="p-1 pt-0 space-y-4">
            ...
            <div className="p-4">
                ...
                <div className="flex items-end">
                    <div className="w-[25%] mb-3 sm:mb-0"> 
                    ...
                    <Button className="ml-5 hover:cursor-pointer" onClick={handleSaveUserHavePermissionMngTimeKeeping}>Save</Button>
                </div>
                ...
                <div className="flex">
                    <div className="border" style={{flexBasis: '25%'}}>
                        ...
                        <GenericAsyncMultiSelect
                            ...
                            onChange={(v) => setCurrentSelectedUser(v as OptionType[])}
                            placeholder="Chọn"
                        />
                    </div>
                            ...
                            <TreeCheckbox
                                ...
                                loadChildren={async (node) => {
                                    if (node.type == "department") {
                                        const children = await orgUnitApi.GetOrgUnitByDept({
                                            departmentId: parseInt(node.id)
                                        })
                                        return children?.data?.data?.map((item: { Id: { toString: () => never; }; Name: never; }) => ({
                                            id: item.Id.toString(),
                                            label: item.Name,
                                            type: "jobtitle"
                                        }));
                    ...
                    <div className="border" style={{flexBasis: '10%'}}>
                        ...
                        <Button onClick={handleSaveUserMngTimeKeeping} className="ml-1 bg-blue-700 hover:bg-blue-800 hover:cursor-pointer">Save</Button>
                    </div>
                </div>
            </div>
        </div>
