...
const ConfigOrgPositionWithScanner: React.FC = () => {
    ...
    const [selectedOrgPosIds, setSelectedOrgPosIds] = useState<string[]>([]);
    const [isLoadingConfig, setIsLoadingConfig] = useState(false);

    const { data } = useQuery({
        queryKey: ['get-page-config-scan-org-pos'],
        queryFn: async () => {
            const res = await scannerMachineApi.getDataPageConfigScanMachineWithOrgPosition();
    ...
    const fetchConfig = useCallback(async (machineId: number) => {
        setIsLoadingConfig(true);
        try {
            const res = await scannerMachineApi.getOrgPositionConfigByScanMachineId(machineId)
            const orgPosIds = res.data.data.map((id: number) => String(id));
            setSelectedOrgPosIds(orgPosIds);
        } catch (error) {
            console.error("Fetch config error:", error);
            setSelectedOrgPosIds([]);
        } finally {
	...
	useEffect(() => {
		if (selectedDeviceIds.length === 1) {
			fetchConfig(selectedDeviceIds[0]);
		} 
	}, [selectedDeviceIds, fetchConfig]);
    ...
    const handleToggleDept = useCallback((deptId: string) => {
        setSelectedOrgPosIds(prev => 
            prev.includes(deptId) ? prev.filter(id => id !== deptId) : [...prev, deptId]
        );
    }, []);

    const saveScanMachineWithOrgPos = useSaveScanMachineWithOrgPosition();

    const handleSave = async () => {
        if (selectedDeviceIds.length === 0 || selectedOrgPosIds.length === 0) return;

        const payload = {
            ScanMachineIds: selectedDeviceIds,
            OrgPositionIds: selectedOrgPosIds.map(id => Number(id))
        };

        await saveScanMachineWithOrgPos.mutateAsync(payload);
    };
        ...
        <div className="flex h-screen p-4 lg:p-2 font-sans text-slate-800 overflow-hidden bg-slate-50">
            <aside className="w-72 lg:w-80 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col mr-6">
                <header className="p-4 border-b">
                    <div className="flex items-center justify-between mb-2">
                        <button onClick={() => { setSelectedDeviceIds([]); setSelectedOrgPosIds([]); }} className="text-[13px] text-indigo-600 hover:underline cursor-pointer">
            ...
            <main className="flex-1 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
				...
				<header className="p-6 border-b border-slate-50 flex items-center justify-between gap-4">
					...
					<button 
						disabled={saveScanMachineWithOrgPos.isPending || !selectedDeviceIds.length || !setSelectedOrgPosIds.length}
						onClick={handleSave}
					...
					>
						{saveScanMachineWithOrgPos.isPending ? <Loader2 size={16} className="animate-spin" /> : (lang === 'vi' ? 'Lưu cấu hình' : 'Save Config')}
					</button>
				</header>
                    ...
                    {selectedDeviceIds.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-400">
                            ...
                            <p className="text-sm font-medium italic">
                                {lang === 'vi' ? 'Chọn ít nhất 1 máy bên trái để xem/gán vị trí' : 'Select at least 1 device to configure'}
                            </p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {data?.orgPositions.map((orgPos: any) => {
                                const active = selectedOrgPosIds.includes(String(orgPos.id));
                                return (
                                    <button 
                                        key={orgPos.id}
...
export default ConfigOrgPositionWithScanner;