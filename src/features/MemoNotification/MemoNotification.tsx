...
export default function MemoNotification () {
    ...
    const { data: MemoNotify, isPending, isError, error } = useQuery({
        ...
        queryFn: async () => {
            const res = await memoNotificationApi.getAll({Page: page, PageSize: pageSize})
    ...
    return (
        <div className="p-1 pl-1 pt-0 space-y-4 list-memo-notify">
            <div className="flex flex-wrap justify-between items-center gap-y-2 gap-x-4 mb-1">
                <h3 className="font-bold text-xl md:text-2xl m-0">Danh sách thông báo</h3>
                <Button asChild className="w-full sm:w-auto">
                    <Link to="/memo-notify/create">Tạo thông báo</Link>
                </Button>
            </div>
            ...
            <div className="table-responsive w-full mt-3">
                <Table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-auto">
                    <thead className="text-sm text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th className="pl-4 py-3 min-w-[180px] dark:text-white">Tiêu đề</th>
                            <th className="px-4 py-3 min-w-[450px]">Nội dung</th>
                            <th className="px-4 py-3 min-w-[200px]">Bộ phận áp dụng</th>
                            <th className="px-4 py-3 min-w-[180px]">Thời gian hiển thị</th>
                            <th className="px-4 py-3 min-w-[100px]">Trạng thái</th>
                            <th className="px-4 py-3 min-w-[100px]">Thời gian tạo</th>
                            <th className="px-4 py-3 min-w-[100px]">Người tạo</th>
                            <th className="px-4 py-3 min-w-[100px]">Hành động</th>
                        </tr>
                    </thead>
                    <tbody>
                        {isPending ? (
                            ...
                            MemoNotify.map((item: IMemoNotify, idx: number) => (
                                <tr key={idx} className="pl-0 pt-0 memo-row bg-white border dark:bg-[#1e1e1e69]">
                                    ...
                                    <td data-label="Bộ phận áp dụng" className="text-black border-b border-[#b1b1b169] px-4 py-4 break-words whitespace-normal dark:text-white">{item.applyAllDepartment ? "Tất cả phòng ban" : "abc, dev, ee"}</td>
                                    ...
                                    <td data-label="Người tạo" className="text-black border-b border-[#b1b1b169] dark:text-white px-4 py-4">{item?.userCodeCreated}</td>
