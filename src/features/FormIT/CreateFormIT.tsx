...
const App = () => {
    ...
    const [itCategoriesSelected, setItCategoriesSelected] = useState<number[]>([]); 
    ...
    const ITRequestFormSchema = z.object({
        ...
        itRequest: z.object({
            ...
            itCategory: z.array(z.number()).nonempty('Phải chọn ít nhất 1 mục'),
...
type ITRequestState = z.infer<typeof ITRequestFormSchema>;
    ...
    const handleCheckboxChangeITCategory = (value: number) => {
        setItCategoriesSelected(prev => {
                                        ...
                                        ItCategories?.map((item: ITCategoryInterface, idx: number) => (
                                            <label key={idx} className="w-[48%] flex items-center space-x-2 cursor-pointer select-none">
                                                <input
                                                    type="checkbox"
                                                    value={item.id}
                                                    checked={itCategoriesSelected.includes(item.id)}
                                                    onChange={() => handleCheckboxChangeITCategory(item.id)}
                                                    className="border-gray-300 checked:bg-black scale-[1.4]"
                                                />
                                                <span>{item.name}</span>
                                            </label>
                                        ))
